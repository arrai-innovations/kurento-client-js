...
const debug = require('debug')(`kurento-client-js:${path.relative(process.cwd(), module.filename)}`);
...
function noop(error, result) {
  if (error) debug('noop error', error);
...
function KurentoClient(ws_uri, options, callback) {
  ...
  function onNotification(message) {
    ...
    if (!object)
      return debug("Unknown object id '" + id + "'", message);
  ...
  var rpc = new RpcBuilder(JsonRPC, options, function (request) {
    if (request instanceof RpcBuilder.RpcNotification) {
      ...
      if (request.duplicated != undefined)
        return debug('Unexpected request:', request);
    ...
    debug('Invalid request instance', request);
  });
  ...
  function sendImplicitTransaction(operations) {
    function callback(error, result) {
      if (error) return debug('Implicit transaction failed', error);
  ...
  function connect(callback) {
    ...
    function updateNotReconnectIfLessThan() {
      ...
      debug("notReconnectIfNumLessThan = " + notReconnectIfNumLessThan);
    }
    ...
    function sendPing() {
      if (enabledPings) {
        ...
        debug("Trying to send ping, but ping is not enabled");
      }
    }
    ...
    re.on('reconnect', function (n, delay) {
      debug('reconnect to server', n, delay, self.sessionId);
